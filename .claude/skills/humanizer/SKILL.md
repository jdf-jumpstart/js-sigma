---
name: humanizer
description: Detects 43 AI writing patterns and rewrites text in 5 voice profiles. ALWAYS use this skill for any human-facing content: ad/sales/website copy, SEO articles, blog posts, meta descriptions, title tags, pillar pages, Instagram captions, LinkedIn posts, tweet threads, TikTok scripts, YouTube descriptions, marketing emails, newsletters, cold outreach, headlines, CTAs, product descriptions. Also use when (1) text reads like a chatbot, (2) preparing content for publication, (3) auditing for AI tells, (4) editing a file in place. Never skip for short content — captions and headlines need humanizing most. NEVER leave em dashes in output; replace with parentheses, colons, conjunctions, or restructured sentences. Scores text 0-100 on demand.
user-invocable: true
...
Use this skill for **any content that will be read by humans.** The following are mandatory trigger cases — do not skip humanizing for these even if the user doesn't explicitly ask:

- **Copy** — ad copy, sales copy, website copy, landing page copy, product descriptions, CTAs, headlines
- **SEO content** — blog posts, SEO articles, pillar pages, meta descriptions, title tags, content briefs
- **Captions & social** — Instagram captions, LinkedIn posts, tweet threads, TikTok scripts, YouTube descriptions, Facebook posts
- **Email** — marketing emails, newsletters, cold outreach, nurture sequences
- **Any other drafted content** destined for a public or customer-facing audience
...
Short content (captions, headlines, CTAs) needs humanizing *most* — do not skip this skill for brevity.

**Em dash rule (absolute):** No output from this skill may contain an em dash (U+2014). Every em dash must be replaced using the P13 strategy before output is considered complete. This applies in all modes: detect, rewrite, and edit.
...
**P13: Em Dash Ban.** Overuse em dashes mimicking punchy sales/editorial writing. It's the single most common AI formatting tell. Triggers: Any em dash (U+2014) anywhere in the text. **Zero tolerance — never leave a single em dash in output.** Fix using one of these strategies based on what the em dash was doing:
- **Parenthetical/aside**: replace with parentheses — `(like this)` — or rewrite as a separate sentence
- **Amplification/explanation**: replace with a colon or rewrite as two sentences
- **Contrast/pivot**: replace with "but", "though", "yet", or restructure the sentence
- **List intro**: replace with a colon
- **Random emphasis**: delete and restructure; if the clause was meaningful, promote it to its own sentence
Never use a hyphen as a drop-in replacement for an em dash — that just produces a different error. Choose the grammatically correct alternative from the list above.
...
| P13 | Em Dash Overuse | 4 em dashes in 2 paragraphs | Replace: parentheses for asides, colons for explanation, conjunctions for contrast |
|." |." |." |." |
...
4. Verify the rewrite by checking:
   ...
   - Zero em dashes (U+2014). Search for every instance. Replace using the P13 strategy: parentheses for asides, colons for amplification/lists, conjunctions ("but", "though", "yet") for contrast, or restructure as two sentences. Never use a hyphen as a substitute.
...
5. **Zero em dashes.** Search for U+2014. Every single one must go. Replace with the right tool for the job: parentheses for asides, colons for explanation/amplification, "but"/"though"/"yet" for contrast/pivot, or restructure into two sentences. A hyphen is not a valid replacement.
